...
const fetchDoctor = async () => {
  ...
  try {
    ...
    // Populate form data
    formData.full_name = docData.value.full_name || ''
    formData.phone = docData.value.phone || ''
    formData.specialization = docData.value.specialization || ''
    formData.qualification = docData.value.qualification || ''
    formData.registration_number = docData.value.registration_number || ''
    formData.experience_years = docData.value.experience_years || 0
    formData.consultation_fee = docData.value.consultation_fee || 0
    formData.department_id = docData.value.department_id || ''
    formData.available_days = Array.isArray(docData.value.available_days)
      ? docData.value.available_days
      : (docData.value.available_days || '').split(',').map(d => d.trim()).filter(d => d)
    formData.start_time = docData.value.start_time || ''
    formData.end_time = docData.value.end_time || ''
    formData.slot_duration_minutes = docData.value.slot_duration_minutes || 15
  } catch (err) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await adminAPI.updateDoctor(authStore.token, props.doctor.id, payload)

    toastMessage.value = 'Doctor updated successfully!'
